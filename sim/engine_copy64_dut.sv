// Simulation-only wiring of blit_copy64 (the paired-pixel BLIT_COPY/
// BLIT_COPY_KEY engine sprite_batch actually drives in production) plus the
// real DDRAM adapter (DDR-003/DDR-007), exposing the real DDRAM_* signal
// names so the testbench can drive one behavioral Avalon-MM memory model
// that must honor DDRAM_BURSTCNT correctly. blit_copy64 has no CMDQ opcode
// of its own -- sprite_batch launches it directly from descriptor fields --
// so start/dst_addr/etc are driven straight from the testbench, matching
// present_dut.sv's raw-port style rather than engine_copy_dut.sv's
// CMDQ-fronted one.

module engine_copy64_dut (
    input  logic          clk,
    input  logic          reset,
